...
public class MyProfile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseDatabase.getInstance().getReference().child("RegisterUser").child(phoneNumber).get().addOnSuccessListener(new OnSuccessListener<DataSnapshot>() {
            @Override
            public void onSuccess(DataSnapshot dataSnapshot) {
                editTextTextPersonName.setText(dataSnapshot.child("name").getValue().toString());
                editTextTextEmailAddress.setText(dataSnapshot.child("email").getValue().toString());
...
//                    }
//                });
                Intent intent = new Intent(MyProfile.this, MyOrders.class);
                                                  ...
                                                  public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                      if(dataSnapshot.hasChild(phoneNumber)){
                                                          try{
                                                              ...
                                                              user1.setPhone(Integer.parseInt(String.valueOf(editTextPhone.getText())));
                                                              user1.setEmail(editTextTextEmailAddress.getText().toString().trim());

                                                              upRef = FirebaseDatabase.getInstance().getReference().child("RegisterUser").child(phoneNumber);
                                          ...
                                          public void onClick(View view) {
                                              ...
                                              delRef.addListenerForSingleValueEvent(new ValueEventListener() {
                                                  ...
                                                  public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                      if(dataSnapshot.hasChild(phoneNumber)){
                                                          try{
                                                              delRef = FirebaseDatabase.getInstance().getReference().child("RegisterUser").child(phoneNumber);
                                                              delRef.removeValue();
                                                              clearControls();
